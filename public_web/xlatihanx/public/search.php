<?php 
    ...
    $search     = htmlentities(isset($_GET['qstr']) ? $_GET['qstr'] : $_POST['searchtext']); 
        ...
        <div class="container-fluid">
            <div class="collapse navbar-collapse">
                <ul class="nav navbar-nav navbar-right">
                    ...
                    <li><a href="../repolatihan/views/external/login.php"><i class="fa fa-user"></i> Latihan</a></li>
                </ul>
            </div>
        </div>
    </nav>
                        ...
                        <?php
                            if($doSearch)
                            {
                                ...
                                {                    
                                    ...
                                    echo '<p class="result-title-2">Kategori : <a href="search.php?qstr=&kat='.urlencode($val['kod_kat']).'&thn=&trs=1&sktr=1&sts=1">'.$val['nama_kat'].'</a></p>';
                                    echo '<p class="result-title-2">Teras : ';
                                    foreach ($val['teras'] as $key2 => $row) 
                                    {
                                        echo '<a href="search.php?qstr=&kat=1&thn=&trs='.urlencode($row['kod_teras']).'&sktr=1&sts=1">'.$row['nama_teras'].'</a>';
                                    }

                                    if ($val['kod_status'] == 2) { 
                                        echo '<p class="result-title-2">Status : Masih Berkuat Kuasa'; 
                                    } elseif ($val['kod_status'] == 3) {
                                        echo '<p class="result-title-2">Status : Telah Dimasuhkan pada '.$date_mansuh->format('d-m-Y');  
                                    ...
                                    echo '</p>';
                                    echo '<div class="result-desc">'.$val['des_dok'];
                        ...
                        <div class="row">
                            ...
                            <?php
                                if($doSearch)
                                {
                                    ...
                                    for ($i=1; $i<=$resPage[1]; $i++) 
                                    {  
                                        echo "<li class=".($i == $page ? "active" : "")."><a ".($i == $page ? "" : "href=search.php?qstr=".urlencode($search)."&kat=".urlencode($kategori)."&thn=".urlencode($tahun)."&trs=".urlencode($teras)."&sktr=".urlencode($sektor)."&sts=".urlencode($status)."&page=".$i).">".$i."</a></li>"; 
                                    }
                                    echo "</ul>";
                                }
                            ?>

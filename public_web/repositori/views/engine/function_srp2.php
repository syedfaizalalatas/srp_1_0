    ...
    if (isset($sql)) {
        ...
        if ($rows_returned == 0) {
            ...
            foreach($arr as $row) {
                ...
                $sql_detail_dok = "SELECT B.nama_kat, C.nama_status, C.kod_status, D.nama_jab FROM dokumen as A, kategori as B, status as C, jabatan as D 
								WHERE A.kod_dok = '$kod_dok_ini' AND A.kod_kat = B.kod_kat AND A.kod_status = C.kod_status AND A.kod_jab = D.kod_jab";  
				...
				//$bil_status = $row_detail_dok['kod_status'];
                if ($row_detail_dok['kod_status'] == 4) {
					
                    $nama_jab_baharu = " ".stripslashes(strtoupper($row_detail_dok['nama_jab']));
                }
                else {
                    $nama_jab_baharu = "";
                }
                if ($row_detail_dok['kod_status'] == 5) {
                    $tajuk_dok_baharu = " ".stripslashes(strtoupper($row_detail_dok['tajuk_dok_baharu']));
                }
                else {
                    $tajuk_dok_baharu = "";
                }
...
function fnShowUpdateDocFormContentForSearch($a,$b,$c,$d,$e,$f,$g){
    $DBServer       = $_SESSION['DBServer'];
    $DBUser         = $_SESSION['DBUser'];
    $DBPass         = $_SESSION['DBPass'];
    $DBName         = $_SESSION['DBName'];
    ...
    if (isset($searchvalue) != "") {
        ...
        foreach($arr as $row) {
            // $temp_nama_data=$row[$field02name];
            //$row[$field02name]=stripslashes("$temp_nama_data");
            //$row[$field02name]=stripslashes($row[$field02name]);
            // $temp_nama_data=$row[$field02name];
            // $temp_nama_data="eh";
            ?>
            <div class='form-group'>
...
function fnVerifyLogin($a,$b,$c,$d,$e,$f){
    ...
    //$conn = new mysqli($DBServer, 'srp', 'srp', 'srp1_0');
	$conn = new mysqli($DBServer, $DBUser, $DBPass, $DBName);
    ...
    if ($rows_returned === 1) {
        ...
        /*if($rs === "false") {
            trigger_error('Wrong SQL: ' . $sql . ' Error: ' . $conn->error, E_USER_ERROR);
            fnRunAlert("Error");
            fnRunAlert("Alert FALSE!");
        } else {
			//$arr = array();
            //$arr = $rs->fetch_all(MYSQLI_ASSOC);

            fnRunAlert("Mendapatkan maklumat.");
            fnRunAlert("Alert TRUE!");
        }*/
		
		//$outp = array();    

		/*while ($row = $rs->fetch_assoc()) {
			fnRunAlert("Alert WOOOOWWWWWWWWWWWW!");
        ...
        foreach($rs as $row) {
			fnRunAlert("Alert WOOOOWWWWWWWWWWWWYESSSSSSS!");
            $_SESSION['loggedinid'] = $row['id_pengguna'];        
            $_SESSION['loggedinname'] = $row['nama_pengguna'];        
            $_SESSION['loggedin_nama_penuh'] = $row['nama_penuh'];
            $_SESSION['status_pentadbir_sistem'] = $row['pentadbir_sistem'];        

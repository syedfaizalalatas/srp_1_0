...
function fnInsertNewDoc($a,$b,$c,$d){
    ...
    /* Bind parameters. TYpes: s = string, i = integer, d = double,  b = blob */
    $stmt->bind_param('siisiiiiiiissssssssiissss',$_SESSION['tajuk_dok'],$_SESSION['bil_dok'],$_SESSION['tahun_dok'],$_SESSION['des_dok'],$_SESSION['kod_kat'],$_SESSION['kod_sektor'],$_SESSION['kod_bah'],$_SESSION['kod_kem'],$_SESSION['kod_jab'],$_SESSION['kod_status'],$_SESSION['id_pendaftar'],$_SESSION['tarikh_wujud'],$_SESSION['tarikh_dok'],$_SESSION['nama_fail_asal'],$_SESSION['nama_fail_disimpan'],$_SESSION['tarikh_kemaskini'],$_SESSION['tarikh_mansuh'],$_SESSION['tarikh_pinda'],$_SESSION['tarikh_serah'],$_SESSION['kod_jab_asal'],$_SESSION['kod_jab_baharu'],$_SESSION['tag_dokumen'],$_SESSION['tajuk_dok_asal'],$_SESSION['tajuk_dok_baharu'],$_SESSION['catatan_dokumen']);
...
function fnShowDocReportByCat($a,$b,$c,$d,$e,$f,$g){
    ...
    if (isset($_SESSION['doclist_search_keyword']) != "") {
        $doclist_search_keyword = $_SESSION['doclist_search_keyword'];
        $sql="SELECT * FROM kategori WHERE nama_kat LIKE '%$doclist_search_keyword%' AND kod_kat != 1 ORDER BY nama_kat ASC";
    }
    else {
    ...
    foreach($arr as $row) {
        $kod_kat = $row['kod_kat'];
        $sql="SELECT kod_kat FROM dokumen WHERE kod_kat = $kod_kat";
        $rs=$conn->query($sql);
        if($rs === false) {
            trigger_error('Wrong SQL: ' . $sql . ' Error: ' . $conn->error, E_USER_ERROR);
        } else {
        ...
        <tr>
            <td>".$counter.".</td>
            <td hidden>".$row[$field01name]."</td>
            <td>".stripslashes(strtoupper($row['nama_kat']))."</td>
            <td style='align-content: center;' align='center'>
                ".$jum_kat_ini."
            </td>
        </tr>
...
function fnShowDocReportByDiv($a,$b,$c,$d,$e,$f,$g){
    ...
    if (isset($_SESSION['doclist_search_keyword']) != "") {
        $doclist_search_keyword = $_SESSION['doclist_search_keyword'];
        $sql="SELECT * FROM bahagian WHERE nama_bahagian LIKE '%$doclist_search_keyword%' AND kod_bah != 1 ORDER BY nama_bahagian ASC";
    }
    else {
        $sql="SELECT * FROM bahagian WHERE kod_bah != 1 ORDER BY nama_bahagian ASC";
    }
    // $sql="SELECT kod_kat, nama_kat FROM kategori WHERE kod_kat != 1 ORDER BY nama_kat ASC";
    ...
    foreach($arr as $row) {
        $kod_bah = $row['kod_bah'];
        $sql="SELECT kod_bah FROM dokumen WHERE kod_bah = $kod_bah";
        $rs=$conn->query($sql);
        if($rs === false) {
            trigger_error('Wrong SQL: ' . $sql . ' Error: ' . $conn->error, E_USER_ERROR);
        } else {
        ...
        <tr>
            <td>".$counter.".</td>
            <td hidden>".$row[$field01name]."</td>
            <td>".stripslashes(strtoupper($row['nama_bahagian']))."</td>
            <td style='align-content: center;' align='center'>
                ".$jum_bah_ini."
            </td>
        </tr>
...
function fnShowDocReportBySec($a,$b,$c,$d,$e,$f,$g){
    ...
    foreach($arr as $row) {
        ...
        $sql="SELECT kod_sektor FROM dokumen WHERE kod_sektor = $kod_sektor";
        $rs=$conn->query($sql);
        if($rs === false) {
            trigger_error('Wrong SQL: ' . $sql . ' Error: ' . $conn->error, E_USER_ERROR);
        } else {
        ...
        <tr>
            ...
            <td>".stripslashes(strtoupper($row['nama_sektor']))."</td>
            <td style='align-content: center;' align='center'>
                ".$jum_sek_ini."
            </td>
        </tr>
...
function fnShowDocReportByStat($a,$b,$c,$d,$e,$f,$g){
    ...
    foreach($arr as $row) {
        ...
        $sql="SELECT kod_status FROM dokumen WHERE kod_status = $kod_status";
        $rs=$conn->query($sql);
        if($rs === false) {
            trigger_error('Wrong SQL: ' . $sql . ' Error: ' . $conn->error, E_USER_ERROR);
        } else {
        ...
        <tr>
            ...
            <td>".stripslashes(strtoupper($row['nama_status']))."</td>
            <td style='align-content: center;' align='center'>
                ".$jum_stat_ini."
            </td>
        </tr>
...
function fnShowDocTableContent($a,$b,$c,$d,$e,$f,$g){
    ...
    foreach($arr as $row) {
        ...
        <tr>
            <td>".$counter.".</td>
            <td hidden>".$row[$field01name]."</td>
            <td>".stripslashes(strtoupper($row['tajuk_dok']))." ".$perkataanBil." ".$bilDok."".$strokeBil."".$row['tahun_dok']."</td>
            <td style='align-content: center;' align='center'>
        ...
        if ($rows_returned == 0) {
            ...
            foreach($arr as $row) {
                ...
                <tr>
                    ...
                    <td>".stripslashes(strtoupper($row['tajuk_dok']))." BIL. ".$row['bil_dok']."/".$row['tahun_dok']."</td>
                    <td style='align-content: center;' align='center'>
                        <button type='submit' id='btn_papar_borang_kemaskini' name='btn_papar_borang_kemaskini' class='btn btn-success' title='Kemaskini' value='".$row['kod_dok']."' hidden><i class='fa fa-edit'></i></button>
                        <button type='submit' id='btn_papar_perincian_dokumen' name='btn_papar_perincian_dokumen' class='btn btn-success' title='Papar' value='".$row['kod_dok']."'><i class='fa fa-eye'></i></button>
...
function fnCountAmmendedDocInRep(){
    ...
    ## Cari kod-kod kategori, nama kategori
    $sql = "SELECT * FROM dokumen WHERE kod_status = '5'";
    ## Bagi setiap kategori, kira bilangan
...
function fnRunConfirm($a){
    ...
    ?>
    <script>
        confirm("<?php echo $_SESSION['msgtoconfirm']; ?>");
    </script>
    <?php
